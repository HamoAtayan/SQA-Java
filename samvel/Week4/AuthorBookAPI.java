...
public class AuthorBookAPI {
    ...
    public static Author[] getAuthors(int length) {
        ...
        for (int i=0; i<length; i++) {
            ...
            authors[i].setAuthor(DataLibrary.names[(int)(Math.random()*4)], DataLibrary.surnames[(int)(Math.random()*4)], (int)(Math.random()*100));
        }
        return authors;
    }
    ...
    public static Book[] getBooks(int length) {
        ...
        for (int i=0; i<length; i++) {
            books[i] = new Book();
            books[i].setBook(DataLibrary.titles[(int)(Math.random()*4)], authors[i], Math.random()*10, createRandomDate(1900, 2000));
        }
        return books;
    }
